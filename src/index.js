...
client.on("interactionCreate", async (interaction) => {
    if (interaction.isChatInputCommand()) {
        // Handles the "crit-hit" slash command
        if (interaction.commandName === "crit-hit") {
            ...
            // Handles the "crit-miss" slash command
        } else if (interaction.commandName === "crit-miss") {
            const level = interaction.options.get("level").value;
            ...
            const advantageType =
                interaction.options.get("advantage-type").value;
            const target = interaction.options.get("target").value;

            await interaction.reply(
                rollGroupSaves(groupSize, saveModifier, advantageType, target)
            );

            // Handles the "group-attacks" slash command
        } else if (interaction.commandName === "group-attacks") {
            const groupSize = interaction.options.get("group-size").value;
            const attackModifier =
                interaction.options.get("attack-modifier").value;
            const advantageType =
                interaction.options.get("advantage-type").value;
            ...
            // Handles the "crit-magic" slash command
        } else if (interaction.commandName === "crit-magic") {
            ...
            await interaction.reply(
                rollCriticalMagic(critType, charLevel, spellLevel)
            );
            // Handles the "find-item" slash command
        } else if (interaction.commandName === "find-item") {
            const rarity = interaction.options.get("rarity").value;
